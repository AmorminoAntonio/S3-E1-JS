...
function concatenaStringhe(s1, s2) {
  const string1 = s1.slice(0, 2);
  const string2 = s2.slice(3, 6);
  return (string1 + string2).toUpperCase();
}
console.log(concatenaStringhe("heila", "ciurma"));
...
const numbers = [2, 7, 10, 42, 8, 3];
const filteredNums = numbers.filter((num) => num % 2 == 0);
...
const incremento = 3;
const risultato = incrementaArray(numeri, incremento);
